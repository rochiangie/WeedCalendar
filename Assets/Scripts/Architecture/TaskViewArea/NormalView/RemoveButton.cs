...
using Assets.SeaTodo.Scripts.MainActivity.MainComponents;
...
namespace Architecture.TaskViewArea.NormalView
{
    ...
    public class RemoveButton : IBehaviorSync
    {
        ...
        // Button text
        private readonly Text removeText;
        // Handle component
        ...
        // Create and setup
        public RemoveButton(float upperSidePosition, Action setupFromPageAction)
        {
            // Save actions
            this.setupFromPageAction = setupFromPageAction;
            
            // Find component and create button component
            rectTransform = SceneResources.Get("ViewFlow Remove Button").GetComponent<RectTransform>();
            mainButtonJob = new MainButtonJob(rectTransform, OpenAccept, rectTransform.Find("Handle").gameObject);
            ...
            removeText = rectTransform.Find("Name").GetComponent<Text>();
            AppTheming.AddElement(removeText, ColorTheme.ViewFlowAreaRemoveButton, AppTheming.AppItem.CreateFlowArea);
            TextLocalization.Instance.AddLocalization(removeText, TextKeysHolder.Remove);
        ...
        private void OpenAccept()
        {
            ...
            // Get title text of accept view
            var acceptTitle = TextLocalization.GetLocalization(TextKeysHolder.AcceptRemove);
            acceptSession = new AcceptSession(mainButtonJob.Reactivate, Remove, acceptTitle);
            AcceptModule.Open(GetWorldAnchoredPosition(), acceptSession);
        }
        ...
        // Get position for accept view when it opens
        private Vector2 GetWorldAnchoredPosition()
        {
            var parent = removeText.transform.parent;

            removeText.transform.SetParent(MainCanvas.RectTransform);
            var result = removeText.rectTransform.anchoredPosition;
            removeText.transform.SetParent(parent);
