...
namespace Architecture.ModuleTrackTime
{
    ...
    public class TrackPositions : IBehaviorSync
    {
        ...
        private const float circlePartMaxDeviation = 0.05f;
        // Deceleration rate while holding the scroll
        private const float slowWhenClampedTouched = 0.07f;
        // Count of containers for a number
        public const int PositionsCount = 15;
        // Convert screen delta to delta of point move in scroll 
        public const float DeltaScreenToMarker = 1f / 500;
        ...
        // Create and setup
        public TrackPositions(RectTransform tracker, TrackScroll scroll, ITrackLine lineTrackSources)
        {
            // Save components
            ...
            // Connect track visual elements to points
            lineTrackSources.SetupElementsToPoints(points);
        ...
        private void CreateMainElements()
        {
            ...
            for (var i = 0; i < PositionsCount; i++)
            {
                ...
                SyncWithBehaviour.Instance.AddObserver(points[i], AppSyncAnchors.TrackTimeModule);
            }
        }
        ...
        {
            var positions = trackerRect.sizeDelta.y; 
        ...
        {
            minLinePosition = lineTrackSources.MinLinePosition();
            maxLinePosition = lineTrackSources.MaxLinePosition();
        }
